...
plot(fib, xlab = "n", ylab = expression(F[n]),
     main = "Sucesión de Fibonacci", pch = 22, cex = 3,
     col = "deeppink1", bg = "gray4", log = "xy")
...
```{r}
plot(n, fib, pch = 21, col = "red", bg = "yellow", cex = 1.7, 
     main = "Fibonacci",
     type = "o", lty = "dotted", lwd = 2, 
     xlim = c(1,11), ylim = c(1, 100),
     xaxp = c(1,10,4), yaxp = c(0,100, 10))
```
...
abline(h = 5, lty = "dotdash", col = "red")
```

```{r}
f <- function(x){x^2}
plot(f, xlim = c(-3,3), col = "red", lwd = 2, ylab = expression(y^2), xlab = "x")
abline(h=0:9, v = -3:3, lty="dotted", col = "black")
```

```{r}
plot(tan, xlim = c(-pi, pi), ylim = c(-5,5))
abline(v = c(-pi/2, pi/2), col = "blue")
```
...
arrows(10,0,40,-10, col = " blue", length = 0.5, angle = 90, code = 3)
arrows(10,0,40,-10, col = " blue", length = 0.5, angle =45, code = 3)
arrows(10,0,40,-10, col = " blue", length = 0.5, angle =25, code = 3)
symbols(40,0,stars = cbind(1,.5,1,.5,1,.5,1,.5,1,.5), add = TRUE, lwd = 4, inches = 0.8)

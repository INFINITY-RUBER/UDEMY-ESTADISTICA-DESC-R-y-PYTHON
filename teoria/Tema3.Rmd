...
x[2:5]= x[2:5]+2
x
x[(length(x)-2):length(x)] = 0  # cambiar las utlmias posiciones
x
x[length(x) + 5] = 9
x
sum(x)
...
y=x
y
y[is.na(y)] = mean(y, na.rm = TRUE)  # REMPLAZA NA POR LA MEDIA DEL VECTOR
y
cumsum(x[!is.na(x)])
```

# OMITIR LOS N/A DEL VECTOR
...
cumsum(na.omit(x))
```
## BORRAR LOS ATRIBUTOS EXTRAS DE OMITR EL N/A

```{r}
x
claer_x = na.omit(x)
claer_x 
attr(claer_x, "na.action") = NULL
claer_x 
attr(claer_x, "class") = NULL
claer_x 
```
...
M[,2]
M[4,]
M[c(1,3,5), 1:2] # crear una subMatriz de M
```
...
t(M)
# para multiplicar matrices %*%:
M%*%t(M)  # 1*1 + 6*6 +  11*11 = 158 Fila*Columna
1*1 + 6*6 +  11*11
M*M
mtx.exp(M,2)
```
...
```{r}
M2 = matrix(c(0,1,0,-7,3,-1,16,-3,4), nrow=3, byrow=TRUE)
M2
eigen(M2)
eigen(M2)$values
eigen(M2)$vectors
...
eigen(A)
solve(A, c(1-1i, 4))
det(A) # el determinate para complejos no existe
